...
- When browser-checking an isolated web UI fixture, serve the fixture and repository assets from the same temporary HTTP origin. Start the server at the repository root, navigate the browser to that origin before injecting or rendering HTML, and load the production stylesheet as `/ui/web/resources/static/site.css`; `file://` and `about:blank` previews do not reliably load local CSS. Stop the temporary server after verification.
